...
requires exactly twelve reports of `Closed under the global context` and rejects `Axiom`,
...
The current milestone has no production Rust code. Adding such code requires a refinement matrix
that maps every public operation and invariant back to a proved contract.
